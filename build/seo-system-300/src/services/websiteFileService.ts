...
export async function updateWebsiteFile(
  fileId: string | number,
  payload: { memo?: string; category?: string }
): Promise<ApiWebsiteFile> {
  return apiJson<ApiWebsiteFile>(API_ENDPOINTS.website, {
    method: 'POST',
    body: JSON.stringify({ action: 'update_file', fileId: Number(fileId), ...payload }),
  });
}

export async function replaceWebsiteFile(
  fileId: string | number,
  file: File,
  memo = '',
  onProgress?: (pct: number) => void
  ...
  form.append('memo', memo);
  form.append('token', token);
  form.append('action', 'replace_file');

  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', `${API_ENDPOINTS.website}?action=replace_file`);
    xhr.withCredentials = true;
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.setRequestHeader('X-CSRF-Token', token);
    xhr.upload.onprogress = (ev) => {
    ...
    xhr.onload = () => {
      try {
        ...
        reject(new Error(body?.error?.message || '파일 교체에 실패했습니다. 다시 시도해주세요.'));
      } catch {
        reject(new Error('파일 교체에 실패했습니다. 다시 시도해주세요.'));
      }
    };
    xhr.onerror = () => reject(new Error('파일 교체에 실패했습니다. 다시 시도해주세요.'));
    xhr.send(form);
  });
}
